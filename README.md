...
Architecture & Flow Diagram - Please refer to attachment

We have three instances
- frontend (NextJS)
- backend (FastAPI)
- scraping (FlareSolverr)
with the laguage model of `gpt4-turbo`. 

**Agent Architecture**

1. We start by understanding user query & extracting filters. 

For this step, we have curate the a few filters by observing Mercari website
...
By predefining schmea, it will reduce the change of language model hallunicating into non-existence filters on Mercari.

2. Product Search - Scraping
...
3. Product Details Enrinchment

While the results from previous step is good, there's not much information to be used for recommendation.
It only have name, price, status etc.

We fetch comprehensive details like description & seller ratings.
...
Fast API is a good fit for this. It also has streaming support (to show as the LLM / system produce the tokens & updates) which is a requirement that I have a mind.
...
Therefore, `o1` and `claude sonnet 3.5` are elimiated as we don't need `Visual Language Model` here.

**OpenAI GPT Family**

I believe product recommendation is something that even `3.5-turbo` can perform very well.

But the challenge is here that currently, 
1. we scraped top 5 products from mercari
2. we let the LLM suggests top 3 that are most relevant to user's query

And we provide the product details information using JSON format.

If we are to increase the candidate products list from 5 for e.g 10 / 20, the context window will easily spike up.
...
**Web Scraping**
Before even chosing any framework `playwright`, `selenium` or native `requests`, I played around with mercari website to see
how their teck stack is doing. 

1. whether it has server side rendering
2. whether it does client side rendering

I noticed mercari is doing client side rendering. Therefore, we can't use `requests`.
We need to use browser automation tool to wait for the `DOM Content` to be loaded.

I started with `playwright`, spinned up docker container and connect to playwright from FastAPI via websocket.

However, mercari has CloudFlare prevention in front of their web portal. I couldn't extract any information.

To bypass the CloudFlare Chanllenge, I bring in `flaresolverr` - https://github.com/FlareSolverr/FlareSolverr
...
I switched to following approach
- `requests` (talks to flaresolver)
...
2. Scraping Performance - when getting the product details, we are fetching sequentially. We can spin up multiple flaresolverr (crawler) nodes and speed up scraping.
3. Candidate selections - our approach is we fetch top K of 5 from Mercari first. Then we ask the LLM to come up with the recommendation. We can definitely increase this number up to ~10-20 with the speed degradation on the other hand. But, this will definitely increase the output quality. This depends on #2 to be able to scale up.
4. Alternatively, we can use `Mercari API` instead of scraping. This will 10x our agent speed. Here is the API - https://api.mercari-shops.com/docs/index.html
5. As I mentioned in evaluation section, we can use `Visual Language Models` / Multi models to look at the product images.